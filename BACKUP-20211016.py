...
class CustomDataset(Dataset):
    ...
    def shows(self, image, label):
        ...
        #print(torch.from_numpy(Labels.colorize(label)))
        # save image
        #plt.imsave(image, os.path.join("output", f"{index}.png"))
        # save label
        #plt.imsave(Labels.colorize(label), os.path.join("output", f"{index}_label.png"))

    def show_dataset(self, sample, index):
        # denormalize sample
        ...
        for idx in range(sample["image"].shape[0]):
            _, axs = plt.subplots(2, figsize=(15, 15))
            # show image and colorized label in one plot 
            axs[0].imshow(transforms.ToPILImage()(sample["image"][idx]))
            axs[0].set_title("Image Data")
            axs[1].imshow(Labels.colorize(sample["label"][idx]))
            axs[1].set_title("Colorized Label")
...
class CustomNormalize(object):
    ...
    def __call__(self, sample) -> dict:
        ...
        image, label = sample["image"], sample["label"]
        #print("@CustomNormalize: image type", type(image))
        ...
        img_trans = trans(image)
        # Transform label from PIL to tensor so image and label has same type
...
class Denormalize(object):
    ...
    def __call__(self, sample) -> dict:
        ...
        # Denormalize image using inverse Normalization
        img_trans = image
        lbl_trans = label 
        for idx in range(image.shape[0]):
            img_trans[idx] = trans(image[idx])
            print(lbl_trans[idx].shape)
            print(np.asarray(transforms.ToPILImage()(label[idx])).shape)
            print(type(torch.tensor(np.asarray(transforms.ToPILImage()(label[idx])))))
            # revert back image and label from Tensor to PIL Image
            img_trans[idx] = torch.tensor(np.asarray(transforms.ToPILImage()(img_trans[idx]))).permute(2, 0, 1)
            lbl_trans[idx] = torch.tensor(np.asarray(transforms.ToPILImage()(label[idx])))

        print("@Denormalize: image type", type(img_trans))
        return {"image":img_trans, "label":lbl_trans}
...
if __name__ == "__main__":
    ...
    for i_batch, sample_batched in enumerate(dataloader):
        ...
        #transformed_dataset.shows(sample_batched["image"], sample_batched["label"], i_batch)
        transformed_dataset.show_dataset(sample_batched, i_batch)
# %%
